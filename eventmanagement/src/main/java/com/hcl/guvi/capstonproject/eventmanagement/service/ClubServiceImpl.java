...
public class ClubServiceImpl implements ClubInterface {
    ...
    @Override
    public List<ClubDto> fetchclubsByOwnerUsername(String clubUserName) {
        List<ClubEntity> clubsEntity = clubRepository.findAllByOwnerName(clubUserName);
        List<ClubDto> listofClubs = clubsEntity.stream().map(entity -> {
            ClubDto club = new ClubDto();
            club.setClubName(entity.getClubName());
            club.setLocation(entity.getLocation());
